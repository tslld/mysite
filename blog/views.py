...
def get_blog_list_common_data(request, blog_all_list):
    # 每页分多少个
    paginator = Paginator(blog_all_list, settings.PER_PAGE_BLOGS_NUMBER) #settings.PER_PAGE_BLOGS_NUMBER
    # 从模板取相应的page参数值
    page_num = request.GET.get('page', 1)  # 获取url的页面参数请求（GET请求）
    # 取出当前page页码的文章数
    ...
    context['blog_types'] = BlogType.objects.annotate(blog_count=Count('blog')) #related在models的外键中
